<template>
<div>
  <h2 class="text-center">预提单转化为正式订单统计</h2>
  ...
  <el-table :data="tableData" border style="width: 100%" :summary-method="getSummaries" show-summary>
    <el-table-column prop="PartitionName" label="大区" width="140">
    </el-table-column>
...
export default {
  ...
  methods: {
    onSearch(params) {
      ...
      getWithholdToOfficial(this.params).then(res => {
    ...
    getSummaries(param) {
      const sums = ['预提单转为正式订单数量：', this.pagination.total, '转化率：', this.cvr]
      return sums
    },
    onDownload(params) {
      const {
      ...
      const url = `/api/download/withholdtoofficial?starttime=${starttime || ''}&endtime=${endtime || ''}&ccodes=${ccodes || ''}&channelname=${channelname || ''}`
      // console.log(url)
      window.open(url)
    }
  },
