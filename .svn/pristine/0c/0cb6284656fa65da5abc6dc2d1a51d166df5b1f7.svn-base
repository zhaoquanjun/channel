<template>
<div>
  <h2 class="text-center">新增客户统计</h2>
  ...
  <el-table :data="tableData" border style="width: 100%" :summary-method="getSummaries" show-summary v-table-sum>
    ...
    <el-table-column prop="ChannelName1" label="一级代理商" min-width="280">
    </el-table-column>
    <el-table-column prop="ChannelName2" label="二级代理商" min-width="290">
    </el-table-column>
    <el-table-column prop="CustomerNum" label="新增客户数" width="130">
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getSummaries(param) {
      const sums = ['合计', '-', '-', '-', '-', this.total.SumCustomerNum, this.total.SumContractAmount]
      return sums
    },
    onDownload(params) {
      const {
      ...
      const url = `/api/download/newcustomercount?starttime=${starttime || ''}&endtime=${endtime || ''}&ccodes=${ccodes || ''}&channelname=${channelname || ''}`
      // console.log(url)
      window.open(url)
    }
  },
