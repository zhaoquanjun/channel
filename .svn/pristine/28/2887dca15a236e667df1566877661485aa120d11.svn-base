...
<div>
  ...
  <SearchParams :length="tableData.length" @search="onSearch" :hide-date="true" @download="onDownload"></SearchParams>
  <el-table :data="tableData" border style="width: 100%" :summary-method="getSummaries" show-summary v-table-sum>
    ...
    <el-table-column prop="BillType" label="已签约" width="130">
    </el-table-column>
    <el-table-column prop="AType" label="准签约(A)" width="130">
    </el-table-column>
    <el-table-column prop="BType" label="重点跟进(B)" width="130">
    </el-table-column>
    <el-table-column prop="CType" label="可跟进(C)" width="130">
    </el-table-column>
    <el-table-column prop="DType" label="有效客户(D)" width="130">
    </el-table-column>
  </el-table>
...
export default {
  ...
  methods: {
    ...
    getSummaries(param) {
      const sums = ['合计', '-', '-', '-', '-', this.total.SumBillType, this.total.SumAType, this.total.SumBType, this.total.SumCType, this.total.SumDType]
      return sums
    },
    onDownload(params) {
      const {
      ...
      const url = `/api/download/customerforcenter?ccodes=${ccodes || ''}&channelname=${channelname || ''}`
      // console.log(url)
      window.open(url)
    }
  },
