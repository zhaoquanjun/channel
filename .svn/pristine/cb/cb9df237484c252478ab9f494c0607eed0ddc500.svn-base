<template>
<div>
  <h2 class="text-center">零申报转化为一般纳税人统计</h2>
...
export default {
  ...
  methods: {
    onSearch(params) {
      ...
      getZerotogeneral(this.params).then(res => {
    ...
    getSummaries(param) {
      const sums = ['零申报转为一般纳税人数量：', this.pagination.total, '转化率：', this.cvr]
      return sums
    },
    onDownload(params) {
      const {
      ...
      const url = `/api/download/zerotogeneral?starttime=${starttime || ''}&endtime=${endtime || ''}&ccodes=${ccodes || ''}&channelname=${channelname || ''}`
      // console.log(url)
      window.open(url)
    }
  },
